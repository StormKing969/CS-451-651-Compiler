...
public class Parser {
    ...
    private boolean seeBasicType() {
        return (see(BOOLEAN) || see(DOUBLE) || see(LONG) || see(CHAR) || see(INT));
    }
    ...
    private boolean seeReferenceType() {
        if (see(IDENTIFIER)) {
            ...
            scanner.recordPosition();
            if (have(BOOLEAN) || have(DOUBLE) || have(CHAR) || have(LONG) || have(INT)) {
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            ...
            mustBe(LPAREN);
            //
            if ((have(INT) || have(DOUBLE) || have(LONG)) && (have(ASSIGN))) {
                // int i = 0;
                JVariableDeclarator declarator = variableDeclarator(type());
                JStatement counter = statement();
                see(SEMI);
                // i <= 10;
                JExpression counter_limit = expression();
                mustBe(SEMI);
                // i++ or i--
                JExpression counter_increment = expression();
                mustBe(RPAREN);
                // The function
                JStatement ForLoopStatement = statement();
                return new JForStatement(line, counter, counter_limit, counter_increment, ForLoopStatement);
            } 
            else {
                JStatement counter = statement();
                see(SEMI);
    ...
    private JExpression assignmentExpression() {
        ...
        }  else if (have(LogicalRShift_ASSIGN)) {
     ...
     *                                 {? true : false conditionalOrExpression}
     * </pre>
     *
    ...
    private JExpression conditionalExpression() {
        ...
        while (more) {
            if (have(COND)) {
                JExpression TrueCase = assignmentExpression();
                mustBe(COLON);
                JExpression FalseCase = conditionalExpression();
                return new JConditionalExpression(line, TrueCase, FalseCase, lhs);
            } else {
                more = false;
            }
        }
     ...
     *                                  {DOUBLE_IOR conditionalAndExpression}
     * </pre>
     *
    ...
    /**
     * Parse an inclusiveOrExpression
     * 
     * <pre>
    ...
    /**
     * Parse an exclusiveOrExpression()
     * 
     * <pre>
     ...
     * 
     * <pre>
     *   AndExpression ::= equalityExpression // level 10
     ...
     *                            {EQUAL | LNOT_ASSIGN relationalExpression}
     * </pre>
     * 
     ...
     *                              [(GT | LE | LESS | GT_ASSIGN) additiveExpression
    ...
    private JExpression relationalExpression() {
        ...
        if (have(GT)) {
            ...
            return new JLessThaOp(line, lhs, shiftExpression());
