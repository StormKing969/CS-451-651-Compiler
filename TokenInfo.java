...
enum TokenKind {
    ...
    // Operators add for Project 1
    DIV("/"), REM("%"), LShift("<<"), RShift(">>"), LogicalRShift(">>>"), UCOM("~"),
    IOR("|"), XOR("^"), AND("&"),

    // Reserved words add for Project 2
    BREAK("break"), CASE("case"), CATCH("catch"), CONTINUE("continue"), DEFAULT("default"), 
    DO("do"), DOUBLE("double"), FINAL("final"), FINALLY("finally"), FOR("for"),
    IMPLEMENTS("implements"), INTERFACE("interface"), LONG("long"), SWITCH("switch"), 
    THROW("throw"), THROWS("throws"), TRY("try"), 

    // Operators add for Project 2
    COND("?"), LNOT_ASSIGN("!="), DIV_ASSIGN("/="), MINUS_ASSIGN("-="), STAR_ASSIGN("*="),
    REM_ASSIGN("%="), RShift_ASSIGN(">>="), LogicalRShift_ASSIGN(">>>="), GT_ASSIGN(">="), 
    LShift_ASSIGN("<<="), LESS("<"), XOR_ASSIGN("^="), IOR_ASSIGN("|="), DOUBLE_IOR("||"),
    AND_ASSIGN("&="),
    ...
    // Literals add for Project 2
    DOUBLE_LITERAL("<DOUBLE_LITERAL>"), LONG_LITERAL("<LONG_LITERAL>");
